...
const TopPicks = () => {
    ...
    return (
        <Grid md={12} color="white">
            <Carousel responsive={responsive} stopAutoPlayOnHover={true}>
                {picks.map(item => (
                    <Box height="430px" width="180px" sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', backgroundColor: '#1a1a1a', mx: 1 }}>
                        <Box>
                            ...
                            <Typography sx={{ m: 1 }}>{item.title.substring(0, 18)}</Typography>
                        </Box>
                        <Box>
                            <Typography sx={{ m: 1 }} className={classes.subtitle}> <Box sx={{ display: 'flex', alignItems: "center" }}> <AddBoxIcon sx={{ mr: 1 }} />Watch options</Box></Typography>
